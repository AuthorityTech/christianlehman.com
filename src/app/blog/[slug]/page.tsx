...
import { renderMarkdownHtml } from "@/lib/markdown-rendering.mjs";
...
export default async function PostPage({ params }: Props) {
  ...
  const share = getPostShare(post);
  const showSectionNav = post.sectionNav.length >= 3;
              ...
              <ol className="space-y-2">
                {post.sectionNav.map((item) => (
                  <li key={item.id} className={item.level === 3 ? "pl-3" : undefined}>
                    <a
                      href={`#${item.id}`}
                      className="block text-[12px] leading-snug text-nothing-secondary transition-colors duration-200 ease-nothing hover:text-nothing-primary"
                    >
                      {item.title}
                    </a>
                  </li>
